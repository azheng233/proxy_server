...
#ifdef JSONRPC_PARAMS_EXTENSION_OFF
	if (*json_params) {
...
json_t *jsonrpc_handle_request_single(json_t *json_request, const struct jsonrpc_method_entry_t *method_table)
{
	int rc;
	...
	json_t *json_result;
	int is_notification;
	const struct jsonrpc_method_entry_t *entry;
...
int jsonrpc_handler(const char *input, size_t input_len,
        const struct jsonrpc_method_entry_t *method_table,
        json_dump_callback_t dump_func, void *dump_param)
{
	...
	if (json_response)
		ret = json_dump_callback(json_response, dump_func, dump_param, JSON_INDENT(1));
