...
int handle(int connect_fd, SSL_CTX *server_ctx, SSL_CTX *client_ctx)
{
	start *line1;
	table *head;
	first *conn_method;
...
fprintf(stderr, "%s %s %s %s\n", line1->method, line1->host, line1->resource, line1->version);
	...
	if (strcmp(line1->method, "CONNECT"))
		err = http_client(target_ip, target_port, connect_fd, line1, head);
	else
		err = ssl_process(target_ip, target_port, connect_fd, line1, head, server_ctx, client_ctx);
