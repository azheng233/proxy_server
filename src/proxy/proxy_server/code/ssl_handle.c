...
int ssl_process(char *ip, char *port, int connect_fd, \
		start *line1, table *hash, \
		SSL_CTX *s_ctx, SSL_CTX *c_ctx, struct link *whitelist)
{
	...
	err = SSL_forward(line1, server_ssl, client_ssl, whitelist);
	if (err) {
		goto end;
	}
end:
