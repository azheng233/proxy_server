...
static int send_request_head(int client_socket, start *line1, table *hash)
{
	char *message;
	int err;	
...
static int forward_response_message(int client_socket, int connect_fd)
{
	...
	while (1) {
		r_num = recv(client_socket, buf, 2047, 0);
...
static int forward_request_entity(int connect_fd, int len, int client_socket)
{
	...
	while (len > 0) {
		...
		if (rnum <= 0) {
			fprintf(stderr, "recv request massage fail\n");
			return -1;
		}
		...
		if (snum <= 0) {
			fprintf(stderr, "send request massage fail\n");
			return -1;
		}
...
static int get_entity_len(table *hash, int connect_fd)
{
	int len, err = 0;
...
static int send_request_entity(int connect_fd, int client_socket, table *hash)
{
	...
	len = get_entity_len(hash, connect_fd);
	if (len) {
